...
export class LocoConnection {
  ...
  async connect(timeoutMs?: number): Promise<void> {
    ...
    this.socket.on("end", () => {
      if (this.responseRejecter) { this.responseRejecter(new Error("Connection closed by server")); this.cleanupResponse(); }
    });
    this.socket.on("error", (err) => {
      if (this.responseRejecter) { this.responseRejecter(err); this.cleanupResponse(); }
    });
  }
  ...
  private onData(chunk: Buffer): void {
    if (this.responseResolver) {
      ...
      // Immediately extract complete frames and notify callbacks
  ...
  private tryResolveResponse(): void {
    ...
    if (payloadSize < 16 || payloadSize > MAX_FRAME_SIZE) {
      this.failResponse(new Error(`invalid frame size: ${payloadSize}`)); return;
    }
    ...
    if (this.responseBuffer.length >= frameSize) {
      ...
      if (remainder.length > 0) this.pushBuffer.push(remainder);
      try {
        this.resolveResponse(decryptLocoFrame(frame, this.sessionKey));
      } catch (error) {
  ...
  async command(packet: Buffer): Promise<Buffer> {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      this.responseBuffer = Buffer.alloc(0);
      this.responseTimer = setTimeout(() => this.failResponse(new Error(`Command timed out after ${this.timeoutMs}ms`)), this.timeoutMs);
      this.socket!.write(encryptLocoFrame(packet, this.sessionKey));
    });
  }

  /** Read buffered push data. Returns decrypted frames. */
  readPushBuffer(): Buffer[] {
    ...
    while (offset < combined.length) {
      if (combined.length - offset < SECURE_FRAME_HEADER_SIZE) { this.pushBuffer.push(combined.subarray(offset)); break; }
      const payloadSize = combined.readUInt32LE(offset);
      if (payloadSize < 16 || payloadSize > MAX_FRAME_SIZE) break;
      const frameSize = 4 + payloadSize;
      if (combined.length - offset < frameSize) { this.pushBuffer.push(combined.subarray(offset)); break; }
      try { results.push(decryptLocoFrame(combined.subarray(offset, offset + frameSize), this.sessionKey)); } catch { /* skip */ }
      offset += frameSize;
    }
    return results;
  }
  ...
  flushPendingAsPush(): void {
    if (this.responseBuffer.length > 0) { this.pushBuffer.push(this.responseBuffer); this.responseBuffer = Buffer.alloc(0); }
  }

  getSessionKey(): Buffer { return this.sessionKey; }
  ...
  isConnected(): boolean { return this.socket !== null && !this.socket.destroyed; }
}